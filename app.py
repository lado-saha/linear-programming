...
def create_tableau(A, b, obj_coeffs, var_names, basis_indices, phase_one=False, original_obj_coeffs_phase2=None, num_artificial_vars=0):
    ...
    # +1 for Cj-Zj row, +1 for RHS column
    ...
    if phase_one:
        ...
        for i, name in enumerate(var_names):
            if name.startswith('a'):
                # Cost of artificial variable is 1 in Phase I obj (min sum of a_i)
                cj_phase1[i] = 1.0
        ...
        # Initial W value (objective function value for phase 1)
...
# Maximize is implied for phase II, phase I is specific
def find_pivot_column(tableau_df, phase_one=False):
    # Exclude RHS, ensure float for comparison
    cj_zj_row = tableau_df.iloc[-1, :-1].values.astype(float)

    # Minimizing sum of artificials (W). Cj-Zj for W is (Cj_art - Zj_art). Enter most positive.
    if phase_one:
        if np.all(cj_zj_row <= EPSILON):
            # Optimal for Phase I (all Cj-Zj <= 0 for min problem form)
            return -1
        ...
        if np.all(cj_zj_row <= EPSILON):
            return -1  # Optimal (all Cj-Zj <= 0 for max problem)
        # Max value in Cj-Zj for entering var
        pivot_col_idx = np.argmax(cj_zj_row)
        if cj_zj_row[pivot_col_idx] <= EPSILON:
            return -1  # Optimal if max is non-positive

    # This is np.intp or similar, cast to int when used for df.columns[idx]
...
def find_pivot_row(tableau_df, pivot_col_idx):
    ...
    ratios = []
    # Stores original indices in the tableau_df (0 to num_constraints-1)
    valid_row_indices = []
        ...
        if pivot_r_idx is not None and pivot_c_idx_as_int is not None:
            ...
            if pivot_c_idx_as_int < len(data.columns) - 1:
                for row_idx in range(len(data.index) - 1):
                    ...
                    s.iloc[row_idx, pivot_c_idx_as_int] = (
                        str(current_style) + ';' if current_style else '') + 'background-color: lightblue;'
...
def solve_simplex_problem(obj_type, obj_coeffs_str, constraints_inputs, num_vars_val, num_constraints_val):
    ...
    if num_a > 0:
        ...
        iteration_ph1 = 0  # Initialize iteration_ph1 to ensure it is always defined
        for iteration_ph1 in range(1, MAX_ITERATIONS + 1):
            ...
            if pivot_col_idx == -1:  # Phase I optimal
                ...
                if abs(phase_one_obj_val) > EPSILON:
                    ...
                    tableau_df = create_tableau(A_for_phase2, b_for_phase2, obj_coeffs_for_tableau,
                                                # current var names in tableau
                                                list(tableau_df.columns[:-1]),
            ...
            pivot_row_idx, ratio_desc = find_pivot_row(
                tableau_df, int(pivot_col_idx))  # Cast pivot_col_idx
            ...
            # pivot_col_idx (np.intp) handled inside
...
with gr.Blocks(theme=gr.themes.Ocean(), css=css) as demo:
    ...
    output_steps_html = gr.HTML(label="Simplex Tableaux and Steps",
                                elem_classes="output_html_class")  # Added elem_classes
    ...
    def assemble_and_solve_problem(obj_type_val, obj_coeffs_val, num_vars_val, num_constraints_val,
        ...
        # Each visible constraint contributes 3 fields
        expected_len_flat_args = num_constraints_int * 3
